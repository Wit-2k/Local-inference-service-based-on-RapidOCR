...
def build_ocr_params() -> dict[str, Any]:
    ...
    for prefix, lang in [("Det", LangDet.CH), ("Rec", LangRec.CH)]:
        params.update(
            {
                f"{prefix}.engine_type": EngineType.ONNXRUNTIME,
    ...
    # ONNX Runtime + CUDA 配置
    prefix = "EngineConfig.onnxruntime"
    params.update(
        {
            f"{prefix}.use_cuda": True,
            f"{prefix}.cuda_ep_cfg.device_id": 0,
            f"{prefix}.cuda_ep_cfg.arena_extend_strategy": "kNextPowerOfTwo",
            f"{prefix}.cuda_ep_cfg.cudnn_conv_algo_search": "EXHAUSTIVE",
            f"{prefix}.cuda_ep_cfg.do_copy_in_default_stream": True,
        }
    )
...
async def lifespan(app: FastAPI):
    ...
    print("✅ OCR 引擎加载完成 (ONNX Runtime + CUDA)")
    yield
    engine = None
    print("👋 OCR 引擎已释放")
